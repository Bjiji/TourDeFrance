...
#        Stage.connection.select("select distinct s.* from stages s join ite_stage_results isr on isr.stage_id = s.id and isr.pos = 1 where s.year LIKE '"+ year_condition + "'")
    ...
    if search[:c_finish_young] == "yes" then
      query = query + " AND ig_stage_results.young_id=runner.id "
    elsif search[:c_finish_young] == "no" then
      query = query + " AND (ig_stage_results.young_id is null OR ig_stage_results.young_id!=runner.id) "
    end
    if search[:c_start_young] == "yes" then
      query = query + " AND ig_stage_results.previous_young=runner.id "
    elsif search[:c_start_young] == "no" then
      query = query + " AND  (ig_stage_results.previous_young is null OR ig_stage_results.previous_young!=runner.id) "
    end
    ...
    elsif search[:c_stage_pos] == "finish" then
      query = query + " AND NOT (isr.pos is null) "
    elsif search[:c_stage_pos] == "nofinish" then
      query = query + " AND ((isr.pos is null)) "
    elsif search[:c_stage_pos] == "dns" then
      query = query + " AND (isr.dns) "
    elsif search[:c_stage_pos] == "dnq" then
      query = query + " AND (isr.dnq) "
    elsif search[:c_stage_pos] == "dnf" then
      query = query + " AND (isr.dnf) "
    end
    if (!(race_team == nil) && !race_team.blank?) then
      query = query + " AND rteam.label LIKE '%" + race_team + "%'"
    end
    if (!(uci == nil) && !uci.blank?) then
      query = query + " AND team.uci LIKE '%" + uci + "%'"
    end
