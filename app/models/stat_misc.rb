...
order by 1 desc limit 25;}, year ]
  ...
  def self.statSuccStages(year)
    sql_array = [ %q{SELECT distinct stages.id as stage1, stages2.id as stage2, stages3.id as stage3, team.id as team, runner.id as runner1, runner2.id as runner2, runner3.id as runner3
 FROM stages
 ...
 LEFT JOIN teams team2 ON team2.id = runner2.team_id
 ...
 LEFT JOIN teams team3 ON team3.id = runner3.team_id
 LEFT JOIN ite_stage_results isr3 ON isr3.stage_id = stages3.id AND runner3.id = isr3.race_runner_id

 WHERE stages.year > ?
 and runner.team_id = runner2.team_id
 and runner.team_id = runner3.team_id
